...
@RequestMapping("/estudante") // define o caminho (endpoint) do controller q estamos trabalhando
@CrossOrigin(origins = "*", allowedHeaders = "*") //padrão de segurança de IP 
public class EstudanteController {
	
	@Autowired // injeção de dependência do EstudanteRepository, implementa o repository no controller
	...
	public ResponseEntity<Estudante> post(@RequestBody Estudante estudante) { // RequestBody envia informação para o corpo da requisição
		return ResponseEntity.status(HttpStatus.CREATED).body(repository.save(estudante)); // CREATED devido ao verbo Post, criar um novo registro
	}
	
	@PutMapping
	public ResponseEntity<Estudante> put(@RequestBody Estudante estudante) { //put atualiza um dado que já existe
		return ResponseEntity.status(HttpStatus.OK).body(repository.save(estudante)); // OK devido ao verbo put, cria um novo recurso ou subsititui uma informação 
	}
